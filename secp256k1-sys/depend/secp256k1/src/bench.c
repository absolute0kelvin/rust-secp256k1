...
static void bench_verify(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(batchverify_rustsecp256k1_v0_10_0_ec_pubkey_parse(data->ctx, &pubkey, data->pubkey, data->pubkeylen) == 1);
        CHECK(batchverify_rustsecp256k1_v0_10_0_ecdsa_signature_parse_der(data->ctx, &sig, data->sig, data->siglen) == 1);
        CHECK(batchverify_rustsecp256k1_v0_10_0_ecdsa_verify(data->ctx, &sig, data->msg, &pubkey) == (i == 0));
        data->sig[data->siglen - 1] ^= (i & 0xFF);
        data->sig[data->siglen - 2] ^= ((i >> 8) & 0xFF);
        data->sig[data->siglen - 3] ^= ((i >> 16) & 0xFF);
    }
...
static void bench_sign_run(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(batchverify_rustsecp256k1_v0_10_0_ecdsa_sign(data->ctx, &signature, data->msg, data->key, NULL, NULL));
        CHECK(batchverify_rustsecp256k1_v0_10_0_ecdsa_signature_serialize_der(data->ctx, sig, &siglen, &signature));
        for (j = 0; j < 32; j++) {
            data->msg[j] = sig[j];
            data->key[j] = sig[j + 32];
        }
...
static void bench_keygen_run(void *arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        size_t len = 33;
        batchverify_rustsecp256k1_v0_10_0_pubkey pubkey;
        CHECK(batchverify_rustsecp256k1_v0_10_0_ec_pubkey_create(data->ctx, &pubkey, data->key));
        CHECK(batchverify_rustsecp256k1_v0_10_0_ec_pubkey_serialize(data->ctx, pub33, &len, &pubkey, SECP256K1_EC_COMPRESSED));
...
/* Check if the user tries to benchmark optional module without building it */
    ...
    data.siglen = 72;
    CHECK(batchverify_rustsecp256k1_v0_10_0_ecdsa_sign(data.ctx, &sig, data.msg, data.key, NULL, NULL));
    CHECK(batchverify_rustsecp256k1_v0_10_0_ecdsa_signature_serialize_der(data.ctx, data.sig, &data.siglen, &sig));
    CHECK(batchverify_rustsecp256k1_v0_10_0_ec_pubkey_create(data.ctx, &pubkey, data.key));
    data.pubkeylen = 33;
    CHECK(batchverify_rustsecp256k1_v0_10_0_ec_pubkey_serialize(data.ctx, data.pubkey, &data.pubkeylen, &pubkey, SECP256K1_EC_COMPRESSED) == 1);
