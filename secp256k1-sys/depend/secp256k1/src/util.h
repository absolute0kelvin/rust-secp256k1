...
typedef struct {
    ...
    const void* data;
} batchverify_rustsecp256k1_v0_10_0_callback;

static SECP256K1_INLINE void batchverify_rustsecp256k1_v0_10_0_callback_call(const batchverify_rustsecp256k1_v0_10_0_callback * const cb, const char * const text) {
...
#else
void batchverify_rustsecp256k1_v0_10_0_default_illegal_callback_fn(const char* str, void* data);
void batchverify_rustsecp256k1_v0_10_0_default_error_callback_fn(const char* str, void* data);
#endif

static const batchverify_rustsecp256k1_v0_10_0_callback default_illegal_callback = {
    batchverify_rustsecp256k1_v0_10_0_default_illegal_callback_fn,
    NULL
};

static const batchverify_rustsecp256k1_v0_10_0_callback default_error_callback = {
    batchverify_rustsecp256k1_v0_10_0_default_error_callback_fn,
    NULL
};
...
static SECP256K1_INLINE void *checked_malloc(const batchverify_rustsecp256k1_v0_10_0_callback* cb, size_t size) {
    void *ret = malloc(size);
    if (ret == NULL) {
        batchverify_rustsecp256k1_v0_10_0_callback_call(cb, "out of memory");
    }
    return ret;
}
...
static SECP256K1_INLINE void batchverify_rustsecp256k1_v0_10_0_memczero(void *s, size_t len, int flag) {
 ...
 */
static SECP256K1_INLINE int batchverify_rustsecp256k1_v0_10_0_memcmp_var(const void *s1, const void *s2, size_t n) {
...
static SECP256K1_INLINE void batchverify_rustsecp256k1_v0_10_0_int_cmov(int *r, const int *a, int flag) {
 ...
 * batchverify_rustsecp256k1_v0_10_0_ctz32_var, but permits it to be tested separately. */
static SECP256K1_INLINE int batchverify_rustsecp256k1_v0_10_0_ctz32_var_debruijn(uint32_t x) {
 ...
 * This function is only intended to be used as fallback for
 * batchverify_rustsecp256k1_v0_10_0_ctz64_var, but permits it to be tested separately. */
static SECP256K1_INLINE int batchverify_rustsecp256k1_v0_10_0_ctz64_var_debruijn(uint64_t x) {
...
/* Determine the number of trailing zero bits in a (non-zero) 32-bit x. */
static SECP256K1_INLINE int batchverify_rustsecp256k1_v0_10_0_ctz32_var(uint32_t x) {
    VERIFY_CHECK(x != 0);
    ...
    return batchverify_rustsecp256k1_v0_10_0_ctz32_var_debruijn(x);
#endif
}

/* Determine the number of trailing zero bits in a (non-zero) 64-bit x. */
static SECP256K1_INLINE int batchverify_rustsecp256k1_v0_10_0_ctz64_var(uint64_t x) {
    VERIFY_CHECK(x != 0);
#if (__has_builtin(__builtin_ctzl) || SECP256K1_GNUC_PREREQ(3,4))
...
#else
    /* If no suitable CTZ builtin is available, use a (variable time) software emulation. */
    return batchverify_rustsecp256k1_v0_10_0_ctz64_var_debruijn(x);
#endif
}

/* Read a uint32_t in big endian */
SECP256K1_INLINE static uint32_t batchverify_rustsecp256k1_v0_10_0_read_be32(const unsigned char* p) {
...
SECP256K1_INLINE static void batchverify_rustsecp256k1_v0_10_0_write_be32(unsigned char* p, uint32_t x) {
    p[3] = x;
    p[2] = x >>  8;
    p[1] = x >> 16;
    p[0] = x >> 24;
...
SECP256K1_INLINE static uint64_t batchverify_rustsecp256k1_v0_10_0_read_be64(const unsigned char* p) {
...
SECP256K1_INLINE static void batchverify_rustsecp256k1_v0_10_0_write_be64(unsigned char* p, uint64_t x) {
    p[7] = x;
    p[6] = x >>  8;
    p[5] = x >> 16;
    p[4] = x >> 24;
